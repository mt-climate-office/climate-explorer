...
wrangle_historical <- function(id, name, element) {
  dat <- glue::glue(
    ...
    # "http://fcfc-mesonet-staging.cfc.umt.edu/blm_api/data/historical/{id}/{element}/"
  ) %>% 
...
wrangle_future <- function(id, variable, type, scenarios) {
  ...
  dat <- glue::glue(
    "http://blm_api/data/future/{id}/{variable}/?table_type={type}&scenarios={scenarios}"
    # "http://fcfc-mesonet-staging.cfc.umt.edu/blm_api/data/future/{id}/{variable}/?table_type={type}"
  ) %>%
    readr::read_csv(show_col_types = FALSE) %>% 
    factor_scenario() 
