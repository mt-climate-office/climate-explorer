...
future_timeseries_text <- function(dat) {
  ...
  txt = glue::glue("{txt} Below, a timeseries plot shows projected changes in  {tolower(legend_title(dat$variable[[1]]))} under these different emission scenarios.")
  return(txt)
}
...
res <- purrr::pmap_chr(
  ...
  knitr::knit_child(text = c(
    ...
    "```{r}",
    "#| echo: false",
    "#| tbl-cap: !expr glue::glue('Projected monthly {legend_title(variable, FALSE)} changes in {params$location_name}')",
